...
public class Projectile : MonoBehaviour
{
    ...
    {
        GameObject newBullet = Instantiate(bullet, fireTransform.position, Quaternion.identity);
    ...
    protected virtual void OnBullet() //스킬이면 오버라이드
    {
        ...
        if (isAttack) //검사하는부분 수정 필요
        {
            Debug.Log(hit.collider.name);
            hit.collider.GetComponent<IDamageable>().OnDamage(GrandParentIAttack.OnAttack(hit), hit);
            Destroy(bullet);   //다 파괴됨
